...
if "bookmark_status" not in st.session_state:
    st.session_state.bookmark_status = {}
# ──────────────────────────────────────────────
# 4. 샘플 데이터
# ──────────────────────────────────────────────
...
def render_card(row, idx, card_class="", show_source_box=False, tab_key=""):
    ...
    """, unsafe_allow_html=True)
    # 읽음 버튼 + 즐겨찾기 버튼 나란히 배치
    is_bookmarked_state = st.session_state.bookmark_status.get(idx, False)
    btn_label  = "📖 안읽음으로 표시" if is_read else "✅ 읽음으로 표시"
    bkmk_label = "⭐ 즐겨찾기 해제" if is_bookmarked_state else "☆ 즐겨찾기 추가"

    col_btn, col_bkmk, col_empty = st.columns([1, 1, 3])
    with col_btn:
    ...
    with col_bkmk:
        if st.button(bkmk_label, key=f"bkmk_btn_{tab_key}_{idx}", use_container_width=True):
            st.session_state.bookmark_status[idx] = not is_bookmarked_state
            try:
                st.rerun()
            except AttributeError:
                st.experimental_rerun()
...
with tab_bookmark:
    ...
    bookmarked = scored_data[
    scored_data.index.map(
        lambda i: st.session_state.bookmark_status.get(i, False)
    )
    ].copy()
